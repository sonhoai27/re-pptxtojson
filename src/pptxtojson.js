...
async function getSlideSize(zip) {
  ...
  return {
    width: parseInt(sldSzAttrs?.['cx'] ?? '0') * FACTOR,
    height: parseInt(sldSzAttrs?.['cy'] ?? '0') * FACTOR,
    defaultTextStyle,
  }
}
...
async function processGroupSpNode(node, warpObj) {
  ...
  const cx = parseInt(xfrmNode['a:ext']['attrs']?.['cx'] ?? '0') * FACTOR
  const cy = parseInt(xfrmNode['a:ext']['attrs']?.['cy'] ?? '0') * FACTOR
  const chcx = parseInt(xfrmNode['a:chExt']['attrs']?.['cx'] ?? '0') * FACTOR
  const chcy = parseInt(xfrmNode['a:chExt']['attrs']?.['cy'] ?? '0') * FACTOR
...
function genShape(node, slideLayoutSpNode, slideMasterSpNode, id, name, idx, type, warpObj) {
  ...
  if (custShapType) {
    ...
    const cx = parseInt(ext?.['cx'] ?? '0') * FACTOR
    const cy = parseInt(ext?.['cy'] ?? '0') * FACTOR
    const w = (parseInt(ext?.['cx'] ?? '0') * FACTOR)
    const h = (parseInt(ext?.['cy'] ?? '0') * FACTOR)
    const cX = (1 / maxX) * w
    const cY = (1 / maxY) * h
    let d = ''
...
function getSize(slideSpNode, slideLayoutSpNode, slideMasterSpNode) {
  ...
  return {
    width: parseInt(ext?.['cx'] ?? '0') * FACTOR,
    height: parseInt(ext?.['cy'] ?? '0') * FACTOR,
  }
}
